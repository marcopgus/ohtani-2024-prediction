---
title: "Shohei Ohtani (The Pitcher) Correct"
...
Create a Data Frame of how would it look vs the Top 5 Pitchers and Total Pitchers 
```{r}
top5_mean <- mean(head(final_stats[order(-final_stats$Weighted_Score2024), ]$Weighted_Score2024, 5))
...
comparison_df <- data.frame(
  Category = c("Top 5 Mean", "Total Mean", "Shohei Ohtani"),
  Score = c(top5_mean, total_mean, ohtani_score)
)
```

Bar Plot comparison between Ohtani, the average of the Top 5 players and the total players average 
```{r}
...
ggplot(comparison_df, aes(x = Score, y = Category, fill = Category)) +
  ...
  labs(
    ...
    subtitle = "Comparison with Top 5 Mean and Total Mean",
...
Scatterplot that helps compare where Ohtani places against other pitchers
```{r}
...
ggplot(top_15, aes(x = Weighted_Score2024, y = reorder(Player, Weighted_Score2024))) +
  ...
  labs(
    title = "Scatter Plot of Top 15 Players' Weighted Score 2024",
    x = "Weighted Score 2024",
    y = "Players"
  ) +
  theme_minimal(base_size = 14)
...
View(final_stats_prediction_comp)
```
...
ggplot(final_stats_prediction_comp, aes(x = Weighted_Score2024, y = Weighted_Score_Prediction)) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", linewidth = 1) +  # Perfect prediction line
  labs(
